...
const m = () => {
  const map = L.map('map').setView([-6.94866, 111.788], 11);
  const key = '7iF1QDaKiFUNiN7SiMMU';
    ...
    const stamenLayer =  L.tileLayer(`https://api.maptiler.com/tiles/5e14fc8f-20b0-4e52-b4df-fc7f73f3ada3/{z}/{x}/{y}.png?key=${key}`,{ //style URL
      tileSize: 512,
      zoomOffset: -1,
      minZoom: 3,
    ...
    const compare = L.control.compare([leftCircle, osmLayer], [rightCircle, stamenLayer]).addTo(map);
}
